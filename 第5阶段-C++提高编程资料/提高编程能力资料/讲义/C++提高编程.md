...
typename  --- 表面其后面的符号是一种数据类型，可以用class代替, **class代表是类的模板，但是函数也可以用**
...
* 模板不能独立使用，必须确定出 T 的类型，但利用显示指定类型的方式，给 T 一个类型，才可以使用该模板
...
2. 可以通过**空模板参数列表**来强制调用函数模板  $\rightarrow$ 即 \<> 
3. **函数模板也可以发生重载**
...
void test01()
{
	...
	//4、 如果函数模板可以产生更好的匹配,优先调用函数模板
    // 虽然上面的 int 类型的 print 函数也可以完成这一功能，但是会先将字符的 ASCII 码转换成整数，较麻烦
    // 因此函数模板是更好的匹配，编译器会调用函数模板来实现
	char c1 = 'a';
	char c2 = 'b';
...
> 1. 普通函数如果声明了却没有实现，调用时还不适用空模板参数列表则会报错

> func\<typename>() 中 \<> 的作用为指定模板函数中 `T` 的类型，想想 Getcompent\<renderer>() 就明白了。
...
// 普通的比较两个对象的模板，不适用于比较自定义的数据类型
template<class T>
bool myCompare(T &a, T &b){
    return a == b ? true : false;
}

// 重载的比较函数，适用于 Person 类型的对象
template<> bool myCompare(Person &a, Person &b){
    // 这里比较有意思，可以继续调用函数模板来实现成员变量的比较
    return (myCompare(a.m_Age, b.m_Age) && myCompare(a.m_Name, b.m_Name)) ? true : false;
}
...
#include <string>
//类模板
template<class NameType, class AgeType = int> // 函数模板不具有此功能
class Person
{
public:
	Person(NameType name, AgeType age)
...
* **vector访问元素时的速度会比deque快**,这和两者内部实现有关（因为要分缓冲区查找）
...
> **总结：**
>
> 1. deque容器和vector容器的构造方式几乎一致，灵活使用即可
> 2. **若函数的参数加入了 const 修饰，则对应的迭代器也应该改为 const_iterator**
...
> 最先入栈的数据的一侧为栈底、最后入栈的数据的一侧为栈顶，栈顶数据最先出栈。
...
> 由于vector有动态扩展的机制，会重新寻找更大的空间来存放数据，因此最初的迭代器有可能失效，而list没有此机制，因而不会失效。
...
> list 容器的排序不能用 Algorithm 中的 sort 算法，原因在于：
>
> **所有不支持随机访问迭代器的容器，都不可以用标准算法**
>
> list 只能用 自己类中的成员函数 sort 排序
...
* `for_each(iterator beg, iterator end, _func);  `
  ...
  > 1. _func 可传入函数或者仿函数
  > 2. for_each 中 _func 传入的是解引用后的数据
...
**总结：**merge合并的两个容器必须为有序序列

> merge 的目标容器也必须提前分配空间
