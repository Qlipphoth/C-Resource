...
int * func()
{
	int a = 10;  // 局部变量存放在栈区，函数执行完之后变量自动释放，无法返回该地址
	return &a;
}

int main() {

	int *p = func();  // 代表返回值是一个地址，p指针用于接收这个返回地址

    // 根据编译器版本，第一次引用可能会报错也可能不会，有的版本会将返回值进行保留，但最好还是不要这样做。
	cout << *p << endl;  // *p 解引用，取得值
	// 第二次必然会回收然后出错
    cout << *p << endl;
...
int* func()
{
    // 这个 a 指针本身是保存在栈区，会自动回收，但是 new 的 int(10) 放在堆区
    // 即 a 中存放的是 new 出的 10 的地址
	int* a = new int(10);
	return a;
}
...
int main() {
    ...
    delete p;  // 可手动回收，那样后面就会报错
    
	cout << *p << endl;  // 打印两次都没有问题，因为堆区的变量不会主动回收
...
public:
	...
	Person& operator=(Person &p)
	{
		...
		m_Age = new int(*p.m_Age);  
         ...
         // 对象在析构时会释放对应地址中的值，这样一来拷贝的对象中指向的地址即变为非法，解决方式为深拷贝，再开辟          
         // 出一块区域用于存储数据
...
void test01()
{
	cout << "sizeof Son = " << sizeof(Son) << endl;  // 16, 说明父类的三个 int 属性全部继承了
}
...
> 父类中所有非静态成员属性都会被子类继承下去，但会被编译器隐藏，所以访问不到
...
4. 如果子类中出现和父类同名的成员函数，子类的同名成员会隐藏掉父类中所有同名成员函数，**包括重载**，想要访问需要**加作用域**
...
> 2. 所有对象共享同一份函数实例
>
> **静态成员可通过类名来直接访问而不用实例化对象**, 如 Person::func();
...
void test02()
{
	...
	cout << "通过类名访问： " << endl;
	Son::func();
	Son::Base::func();  // Son 作用域下的 Base 作用域下的 func()
...
2. 草泥马**继承自动物的数据继承了两份**，其实我们应该清楚，这份数据我们只需要一份就可以。
...
void test01()
{
	...
	cout << "st.m_Age = " << st.m_Age << endl;  // 如果不为虚继承则这里会有二义性
}
...
>
> 虚继承解决这个问题的方式是使得子类继承一个指针，叫做vbptr，即 virtual base pointer，该指针指向最后一次被赋值的虚基类成员？
...
public:
    ...
    // 虚函数的实质是由 vfptr（虚函数指针）指向了虚函数表中的一个函数地址
	virtual void speak()
	{
		cout << "动物在说话" << endl;
	}
...
// 子类继承父类时，虚函数被一起继承，指向父类的虚函数地址
class Cat :public Animal
{
public:
    // 子类重写父类的虚函数时，会使得虚函数指针被覆盖，变为子类的对应函数地址
	void speak()
	{
		cout << "小猫在说话" << endl;
	}
...
// 多态的重点是传入函数的是父类的引用或指针（因为不确定调用时会传入哪个子类），但是想在调用时
// 分别调用不同的子类内部重写了的函数，这时就要将父类的对应函数写为虚函数，以实现调用时根据传入对象的类别来
// 动态地选择，即实现多态
...
> **C++ 中父类的引用可以直接指向子类的对象，即子类对象的类型既是它本身也是父类类型**
...
public:
	...
	// 子类在重写析构函数时，虚函数指针就能指向子类的析构函数，从而将子类申请的对象释放掉
	virtual ~Animal() = 0;
};

// 即使是纯虚析构函数也需要有具体的代码实现
Animal::~Animal()
{
